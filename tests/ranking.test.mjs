...
test('daily leaderboard carries the pre-race racer name', () => {
  const leaderboard = buildDailyLeaderboard([
    {
      id: 'named',
      playerId: 'a',
      publicRacerId: 'GUZ-000001',
      displayName: 'Basement Bandit',
      correctAnswers: 12,
      totalPuzzles: 12,
      completionTimeMs: 150000,
      submittedAt: '2026-07-30T12:00:00Z',
    },
  ], 'GUZ-000001');

  assert.equal(leaderboard.entries[0].displayName, 'Basement Bandit');
  assert.equal(leaderboard.playerEntry.displayName, 'Basement Bandit');
});
